...
export default function EditBannerPage() {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      ...
      const options = [
        ...
        // Exclude the current banner from being placed after itself
        ...bans.filter(b => b.id !== bannerId).map(b => ({ value: `after_banner_${b.id}`, label: `After banner: ${b.title}`, group: 'After a Banner' }))
      ];
      ...
      const fetchBanner = async () => {
        ...
        if (error) {
          ...
          setForm({
            ...
            banner_size: data.banner_size || "full_width",
            text_alignment: data.text_alignment || "left",
            text_color: data.text_color || "white",
            placement_after: data.placement_after || "top",
          });
        }
        setIsLoading(false);
      };
  ...
  if (isLoading) {
    return (
      <main className="p-8 bg-surface min-h-screen flex items-center justify-center">
        <div className="animate-pulse flex items-center justify-center w-12 h-12 rounded-full border-4 border-indigo-200 border-t-indigo-600"></div>
      </main>
    );
  }
    ...
    <main className="p-8 bg-surface min-h-screen">
      ...
      <div className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-on-surface tracking-tight">Edit Banner</h1>
            <p className="text-sm text-on-surface-variant mt-1">Update promotional banner details.</p>
          </div>
        </div>
